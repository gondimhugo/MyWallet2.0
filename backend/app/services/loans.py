...
def months_between(start: date, end: date) -> int:
    ...
    # Preserve month-end rollover as a full accrual month.
    # Example: Jan/31 -> Feb/28 should count as 1 month.
    end_month_last_day = monthrange(end.year, end.month)[1]
    expected_day = min(start.day, end_month_last_day)
    if end.day < expected_day:
        months -= 1
    return max(0, months)


def _compound_with_period_rounding(principal: float, rate: float, months: int) -> float:
    """Compound balance rounding each period like schedule()."""
    balance = principal
...
def expected_return(loan: Loan) -> float:
    ...
    if loan.interest_mode == LoanInterestMode.compound:
        return _compound_with_period_rounding(principal, rate, n)
